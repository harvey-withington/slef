...
export class TemplateGenerator {
  ...
  generateAlgorithmParams(seedId) {
    ...
    while (!isValid) {
      ...
      // Validation Rules
      const hasMixer =
        sequence.includes("substitute") || sequence.includes("xor");
      const hasDoubleReverse = sequence.some(
        (op, i) => op === "reverse" && sequence[i + 1] === "reverse",
      );
  ...
  addUISheet(params) {
    ...
    const finalAlgCol = String.fromCharCode(
      65 + params.operationSequence.length,
    ); // If 3 ops, starts A(input), B(op1), C(op2), D(op3). D is final.
  ...
  addAlgorithmSheet(params) {
    ...
    params.operationSequence.forEach((op, i) => {
      sheet.getCell(`${String.fromCharCode(67 + i)}1`).value =
        `Step ${i + 1}: ${op}`;
    });
    ...
    // =MID($B$2, $A10, 1)
    for (let i = 0; i < MAX_CHARS; i++) {
      sheet.getCell(`B${10 + i}`).value = {
        formula: `=MID($B$2, $A${10 + i}, 1)`,
      };
    }
    ...
    // F10 = E10
    // F11 = F10 & E11
    sheet.getCell(`${accumColLetter}10`).value = {
      formula: `=${finalEncCol}10`,
    };
    for (let i = 1; i < MAX_CHARS; i++) {
      const row = 10 + i;
      sheet.getCell(`${accumColLetter}${row}`).value = {
        formula: `=${accumColLetter}${row - 1} & ${finalEncCol}${row}`,
      };
    }
    ...
    sheet.getCell(outputCellRef).value = {
      formula: `=${accumColLetter}${10 + MAX_CHARS - 1}`,
    };
    ...
    for (let i = 0; i < MAX_CHARS; i++) {
      sheet.getCell(`B${310 + i}`).value = {
        formula: `=MID($B$4, $A${310 + i}, 1)`,
      };
    }
    ...
    sheet.getCell(`${decAccumColLetter}310`).value = {
      formula: `=${finalDecCol}310`,
    };
    for (let i = 1; i < MAX_CHARS; i++) {
      const row = 310 + i;
      sheet.getCell(`${decAccumColLetter}${row}`).value = {
        formula: `=${decAccumColLetter}${row - 1} & ${finalDecCol}${row}`,
      };
    }

    const outputDecCellRef = `${String.fromCharCode(65 + params.operationSequence.length)}5`;
    sheet.getCell(outputDecCellRef).value = {
      formula: `=${decAccumColLetter}${310 + MAX_CHARS - 1}`,
    };
  }
